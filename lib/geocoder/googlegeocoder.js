...
GoogleGeocoder.prototype._formatResult = function(result) {
    ...
    if (this.options.includePlaceId && result.place_id) {
        processedResult.place_id = result.place_id;
    }

    if (this.options.includeFormattedAddress && result.formatted_address) {
        processedResult.formatted_address = result.formatted_address;
    }
