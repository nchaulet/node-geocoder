...
AbstractGeocoder.prototype.geocode = function(value, callback) {
    ...
    if (net.isIPv4(value) && (!this.supportIPv4 || this.supportIPv4 == 'undefined')) {
        throw new Error(this.name + ' no suport geocoding IPv4');
    }

    if (net.isIPv6(value) && (!this.supportIPv6 || this.supportIPv6 == 'undefined')) {
        throw new Error(this.name + ' no suport geocoding IPv6');
    }

    if (this.supportAdress === false && (!net.isIPv4(value) && !net.isIPv6(value))) {
        throw new Error(this.name + ' no suport geocoding adress');
    }
